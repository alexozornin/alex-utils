...
class Decimal {
     ...
     * @param {boolean} [autoFloor] If set to true, exceeding fracture part will be floored to 15 digits instead of throwing an error (this is a destructive operation due to potential loss)
     */
    constructor(num, autoFloor = false) {
        this.private = {
            _sign: 0,
            _integer: 0,
            _fracture: 0
        ...
        if (autoFloor) {
            _fracture_string = _fracture_string.substr(0, 15);
        }
    ...
    toString() {
        ...
        let _fracture_string = `${Math.abs(this.private._fracture)}`;
        while (_fracture_string.length < 15) {
            _fracture_string = `0${_fracture_string}`;
        }
        _fracture_string = _fracture_string.replace(/0+$/, '');
    ...
    round(offset = 0, modify = false) {
        ...
        if (fracDiv.remainder == middle || (this.private._sign ^ fracDiv.remainder > middle)) {
            return this.ceil(offset, modify);
        }
        else {
    ...
    add(num, modify = false) {
        ...
        if (_sign && !_fracture_sign && _fracture != 0) {
            ++_integer;
            _fracture -= 1e15;
        }
        if (!_sign && _fracture_sign && _fracture != 0) {
